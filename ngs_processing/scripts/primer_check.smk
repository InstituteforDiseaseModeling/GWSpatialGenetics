...
rule genome_coverage:
    ...
    output: join(PROJECT_DIR, "01_processing/02_align/coverage/{ena_id}_pairAligned_coverage.txt"),
    threads: 1
    shell: """
...
#     output: join(PROJECT_DIR, "01_processing/02_align/primer_counts/{ena_id}_primerCounts_mismatch{mismatch}.txt")
#     params:
...
rule filter_fastq:
    input: rules.align_to_ref.output
    output: temp(join(PROJECT_DIR, "01_processing/02_align/{ena_id}_pairAligned_filt.fq"))
    params:
        map_qual = config['trim_galore']['quality']
    threads: 4
    shell: """
...
rule primer_counts:
    ...
    output: join(PROJECT_DIR, "01_processing/02_align/primer_counts/{ena_id}_primerCounts.txt")
    params:
        map_qual = config['trim_galore']['quality']
    threads: 2
    shell: """
...
rule primer_summary:
    ...
    output: join(PROJECT_DIR, "01_processing/02_align/primer_counts/{ena_id}_primerCounts_summary.txt")
    threads: 1
    shell: """
