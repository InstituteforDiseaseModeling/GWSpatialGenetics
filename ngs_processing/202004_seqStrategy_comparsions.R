...
gatk_sim <- rbind.data.frame(
  bc_similarity(gatk_missingRm) %>% dplyr::mutate(region="All loci"),
  bc_similarity(dplyr::filter(gatk_missingRm, bc_region == "True")) %>% dplyr::mutate(region="Within loci"),
  bc_similarity(dplyr::filter(gatk_missingRm, bc_region == "False")) %>% dplyr::mutate(region="Outside loci")
)
# populate the number of variants
...
# ggsave("20200515_seqStrategy_gatkParams_rm5950-6670.png", plot = sim_pdf, path = gw_dir, width = 5.5, height = 4, units = c("in"), dpi = 300)
